...
function getFunnel(siteSlug,campaign){return FUNNELS[`${clean(siteSlug,160)}/${clean(campaign,160)}`]||null}
function scoreSubmission(input={}){const a=input.answers||{};let score=0;const reasons=[];if(a.maturity==="reservation-prochaine"){score+=40;reasons.push("reservation-prochaine")}else if(a.maturity==="comparaison"){score+=22;reasons.push("comparaison-active")}else if(a.maturity==="idees"){score+=8;reasons.push("inspiration")}const bp={"5000-plus":24,"3000-5000":20,"2000-3000":14,"moins-2000":8,"a-definir":3};const b=bp[a.budgetPerPerson]||0;score+=b;if(b)reasons.push(`budget:${a.budgetPerPerson}`);if(a.departureWindow&&a.departureWindow!=="pas-encore-decide"){score+=14;reasons.push("periode-definie")}if(a.departureAirport&&a.departureAirport!=="a-definir"){score+=6;reasons.push("depart-defini")}if(a.travelStyle&&a.travelStyle!=="a-decouvrir"){score+=6;reasons.push("style-defini")}if(input.phone&&input.consents?.phoneProjectContact===true){score+=10;reasons.push("rappel-demande")}score=Math.min(score,100);const temperature=score>=70?"HOT":score>=40?"WARM":"COLD";return{score,temperature,recommendedAction:temperature==="HOT"?"CONTACT_PRIORITY":temperature==="WARM"?"CONTACT_OR_NURTURE":"NURTURE",reasons}}
function buildConsentEvidence(input={},context={}){return{version:CONSENT_VERSION,capturedAt:context.capturedAt||new Date().toISOString(),source:clean(context.source||"acquisition-funnel",120),funnelId:clean(context.funnelId,180),siteSlug:clean(context.siteSlug,160),emailMarketing:input.emailMarketing===true,phoneProjectContact:input.phoneProjectContact===true,phoneProvided:Boolean(clean(context.phone,50)),wording:{...CONSENT_WORDING}}}
function buildErpLeadEnvelope({leadId,funnel,submission,qualification,consentEvidence}){return{schemaVersion:"mondescale.acquisition-lead.v1",eventType:"ACQUISITION_LEAD_QUALIFIED",leadId:clean(leadId,160),occurredAt:new Date().toISOString(),agency:{siteSlug:funnel.siteSlug,publicSiteSlug:funnel.publicSiteSlug,city:funnel.agencyCity},campaign:{id:funnel.campaign,funnelId:funnel.id,funnelVersion:funnel.version},contact:{name:clean(submission.name,120),email:clean(submission.email,180).toLowerCase(),phone:clean(submission.phone,50)||null,postalCode:clean(submission.postalCode,20)},project:submission.answers||{},qualification,consentEvidence}}
class DisabledErpConnector{async publish(envelope){return{status:"DISABLED",externalId:null,schemaVersion:envelope.schemaVersion}}}
