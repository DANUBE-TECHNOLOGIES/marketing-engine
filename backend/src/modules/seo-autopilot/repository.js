...
class SeoAutopilotRepository extends TenantScopedRepository {
  ...
  listRuns({ siteId, status, mode, limit = 50 } = {}) { return this.prisma.seoAutopilotRun.findMany({ where: this.scope({ ...(siteId ? { siteId } : {}), ...(status ? { status } : {}), ...(mode ? { mode } : {}) }), orderBy: { createdAt: "desc" }, take: Math.max(1, Math.min(200, Number(limit || 50))) }); }
