...
async function main(){
  ...
  const cols=await prisma.$queryRawUnsafe(`SELECT column_name,is_nullable FROM information_schema.columns WHERE table_schema='public' AND table_name='PublicLead'`);
  ...
  const phoneColumn=cols.find(x=>x.column_name==="phone");
  push("lead-phone-optional",phoneColumn?.is_nullable==="YES",phoneColumn?`phone is_nullable=${phoneColumn.is_nullable}`:"phone column missing");
