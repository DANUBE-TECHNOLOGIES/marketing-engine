...
function exactHeroTarget(page = {}) {
  ...
  const hero = heroes[0];
  return { scope: "block", blockType: "hero", blockId: hero.id ?? null, field: "title" };
}
...
function sealOperationFinalValue(operation = {}, site = {}, page = {}) {
  if (operation.type === "strengthen-title") return { ...operation, target: { scope: "page", field: "seoTitle" }, sourceValueFingerprint: sha256Text(page.seoTitle ?? ""), finalValue: titleForPage({ agency: site.agency || {}, page }) };
  if (operation.type === "strengthen-meta-description") return { ...operation, target: { scope: "page", field: "metaDescription" }, sourceValueFingerprint: sha256Text(page.metaDescription ?? page.seoDescription ?? ""), finalValue: descriptionForPage({ agency: site.agency || {}, page }) };
  if (operation.type === "strengthen-h1") {
    const target = exactHeroTarget(page);
    const sourceValue = exactHeroSourceValue(page, target);
    return { ...operation, target, sourceValueFingerprint: sourceValue === null ? null : sha256Text(sourceValue), finalValue: buildHeroTitle({ agency: site.agency || {}, page }) };
  }
  return { ...operation };
}
...
function exactInternalLinkSource(site = {}, operation = {}) {
  for (const sourceSlug of operation.suggestedSourceSlugs || []) {
    ...
    if (!page) continue;
    const block = (page.blocks || []).find((item) => item?.id !== null && item?.id !== undefined && normalizedBlockType(item).includes("rich-text") && typeof item?.content?.html === "string");
...
function internalLinkLabel(targetPage = {}, site = {}) {
  const profile = pageProfile(targetPage);
  ...
  const labels = {
    ...
    generic: [`Découvrir ${String(targetPage.title || targetPage.slug || "cette page").trim()}`],
  };
  const values = labels[profile] || labels.generic;
  const seed = `${site.agencyId || site.agency?.id || site.slug || ""}|${targetPage.slug || targetPage.title || ""}|internal-link`;
...
function sealInternalLinkOperation(operation = {}, proposal = {}, site = {}, targetPage = {}) {
  ...
  const label = internalLinkLabel(targetPage, site);
  const linkHtml = `<p><a href="${escapeHtml(href)}">${escapeHtml(label)}</a></p>`;
  return {
    ...operation,
    target: { scope: "block", pageSlug: source.page.slug || null, blockType: "rich_text", blockId: source.block.id, field: "content.html" },
...
function proposalWithCopyPreview(proposal, action, site) {
  ...
  return {
    ...proposal,
    operations: page ? (proposal.operations || []).map((operation) => operation.type === "add-internal-link" ? sealInternalLinkOperation(operation, proposal, site, page) : sealOperationFinalValue(operation, site, page)) : [...(proposal.operations || [])],
...
function projectedPages(agencies = []) {
  return agencies.flatMap((agency) => (agency.impact?.pages || []).map((page) => ({ agencyId: agency.agencyId || null, siteSlug: agency.siteSlug || null, ...page })));
}
...
function installQualityUpliftPreview(ServiceClass) {
  ...
  ServiceClass.prototype.previewAgencyQualityUplift = async function previewAgencyQualityUplift({ agencyId, minimumWords = 120 } = {}) {
    ...
    const planFingerprint = qualityUpliftFingerprint({ version: "mse-25.31", siteSlug: plan.siteSlug, agencyId: plan.agencyId, minimumWords: Number(minimumWords || 120), actions: actionPlan.actions });
    return {
      operation: "preview-quality-uplift", writes: false, destructive: false, readOnly: true, minimumWords: Number(minimumWords || 120), ...plan, planFingerprint,
      actions: actionPlan.actions,
      actionSummary: { actionCount: actionPlan.actionCount, highPriorityCount: actionPlan.highPriorityCount, mediumPriorityCount: actionPlan.mediumPriorityCount, lowPriorityCount: actionPlan.lowPriorityCount },
      proposals,
      proposalSummary: {
        ...
        exactMetadataValueCount: proposals.reduce((count, proposal) => count + (proposal.operations || []).filter((operation) => ["strengthen-title", "strengthen-meta-description", "strengthen-h1"].includes(operation.type) && String(operation.finalValue || "").trim()).length, 0),
        exactInternalLinkValueCount: proposals.reduce((count, proposal) => count + (proposal.operations || []).filter((operation) => operation.type === "add-internal-link" && String(operation.finalValue || "").trim() && operation.target?.blockId !== null && operation.target?.blockId !== undefined).length, 0),
      },
      impact,
  ...
  ServiceClass.prototype.previewNetworkQualityUplift = async function previewNetworkQualityUplift({ minimumWords = 120 } = {}) {
    ...
    const excludedSites = (sites || []).filter((site) => String(site?.status || "").toLowerCase() !== "published").map((site) => ({ agencyId: site.agencyId || site.agency?.id || null, siteSlug: site.slug || null, status: site.status || null, reason: "site-not-published" }));
    const agencies = [];
    ...
    const response = {
      version: "mse-25.31", operation: "preview-network-quality-uplift", writes: false, destructive: false, readOnly: true, minimumWords: Number(minimumWords || 120), agencies, excludedSites,
      summary: {
        agenciesProcessed: agencies.length, agenciesExcluded: excludedSites.length,
        intentOpportunityCount: sum(agencies, ["summary", "intentOpportunityCount"]), thinContentOpportunityCount: sum(agencies, ["summary", "thinContentOpportunityCount"]), internalLinkOpportunityCount: sum(agencies, ["summary", "internalLinkOpportunityCount"]), totalOpportunityCount: sum(agencies, ["summary", "totalOpportunityCount"]),
        pageActionCount: sum(agencies, ["actionSummary", "actionCount"]), highPriorityPageCount: sum(agencies, ["actionSummary", "highPriorityCount"]), mediumPriorityPageCount: sum(agencies, ["actionSummary", "mediumPriorityCount"]), lowPriorityPageCount: sum(agencies, ["actionSummary", "lowPriorityCount"]),
        proposalCount: sum(agencies, ["proposalSummary", "proposalCount"]), proposedOperationCount: sum(agencies, ["proposalSummary", "operationCount"]), bodyCopyPreviewCount: sum(agencies, ["proposalSummary", "bodyCopyPreviewCount"]), exactMetadataValueCount: sum(agencies, ["proposalSummary", "exactMetadataValueCount"]), exactInternalLinkValueCount: sum(agencies, ["proposalSummary", "exactInternalLinkValueCount"]),
        projectedOpportunityCount: sum(agencies, ["impact", "projected", "total"]), projectedWarningReduction: sum(agencies, ["impact", "projectedReduction", "total"]), projectionCompleteAgencyCount: agencies.filter((agency) => agency.impact?.projectionComplete === true).length,
        projectedPageCount: pages.length, projectionCompletePageCount: pages.filter((page) => page.projectionComplete === true).length, projectionPartialPageCount: pages.filter((page) => page.projectionComplete !== true).length, pagesWithProjectedReductionCount: pages.filter((page) => Number(page.projectedReduction || 0) > 0).length, fullyResolvedPageCount: pages.filter((page) => Number(page.beforeWarnings || 0) > 0 && Number(page.projectedWarnings || 0) === 0).length,
      },
    };
...
module.exports = { exactHeroSourceValue, exactHeroTarget, exactInternalLinkSource, installQualityUpliftPreview, internalLinkLabel, normalizedBlockType, projectedPages, proposalWithCopyPreview, sealInternalLinkOperation, sealOperationFinalValue, sha256Text, siteFromAgencyPlan, sum };
