...
function buildPageCreateData(composedPage, siteId) {
  if (!composedPage || typeof composedPage !== "object") throw new Error("A composed page is required");
  if (!siteId) throw new Error("A site id is required");
  const { sections = [], ...page } = composedPage;
  return {
    ...page,
    site: { connect: { id: siteId } },
    sections: { create: asArray(sections) },
  };
}

module.exports = { DEFAULT_ORDER, buildContext, buildRecommendations, composeDestinationSections, composeDestinationPage, buildPageCreateData };
