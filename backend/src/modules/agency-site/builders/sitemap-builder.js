...
function isPublishedPage(page) {
  return page?.published === true || page?.status === "published";
}
...
class SitemapBuilder {
  build(site, pages, origin = "https://www.mondescale.fr") {
    ...
    const urls = (Array.isArray(pages) ? pages : [])
      ...
      .map(p => `  <url><loc>${xmlEscape(cleanOrigin + p.path)}</loc><changefreq>${p.pageType === "HOME" ? "weekly" : "monthly"}</changefreq><priority>${p.pageType === "HOME" ? "1.0" : "0.7"}</priority></url>`)
      .join("\n");
...
module.exports.isPublishedPage = isPublishedPage;