...
function sendError(response, error) {
  response.status(statusCode(error)).json({ error: error?.code || "AGENCY_LAUNCH_ERROR", message: error?.message || "Impossible de calculer l'état de lancement." });
}

async function tenantIdForRequest(database, request) {
  const direct = String(request.tenantId || request?.tenant?.id || request.get("x-tenant-id") || "").trim();
  if (direct) return direct;
  const slug = String(request.tenantSlug || request?.tenant?.slug || request.get("x-tenant-slug") || "").trim();
  if (!slug) throw createHttpError(400, "AGENCY_LAUNCH_TENANT_REQUIRED", "Le tenant est obligatoire.");
  const tenant = await database.tenant.findUnique({ where: { slug }, select: { id: true } });
  if (!tenant) throw createHttpError(404, "AGENCY_LAUNCH_TENANT_NOT_FOUND", "Tenant introuvable.");
  return tenant.id;
}

async function assertAgencyInTenant(database, tenantId, agencyId) {
  const id = Number(agencyId);
  if (!Number.isInteger(id) || id <= 0) throw createHttpError(400, "AGENCY_LAUNCH_INVALID_AGENCY_ID", "Identifiant agence invalide.");
  const agency = await database.agency.findFirst({ where: { id, tenantId }, select: { id: true } });
  if (!agency) throw createHttpError(404, "AGENCY_LAUNCH_AGENCY_NOT_FOUND", "Agence introuvable dans ce tenant.");
  return agency.id;
}
...
async function readinessWithPublicRuntime(database, tenantId, agencyId, service) {
  ...
  const [pages, recentHistory] = await Promise.all([
    database.agencySitePage.findMany({
      ...
      select: {
        id: true, slug: true, title: true, h1: true, status: true, published: true, seoTitle: true, metaDescription: true,
        blocks: { orderBy: { displayOrder: "asc" }, select: { status: true, content: true } },
        sections: { orderBy: { displayOrder: "asc" }, select: { status: true, jsonContent: true } },
      },
    }),
...
async function networkForTenant(database, tenantId) {
  ...
  for (const agency of agencies) {
    try {
      ...
      items.push({ agency: { id: agency.id }, readiness: { ready: false }, launchState: { code: "to_complete", label: "À compléter", priority: 2, actionable: true, action: "complete" }, error: { code: error?.code || "AGENCY_LAUNCH_READINESS_ERROR", message: error?.message || "État de lancement indisponible." } });
    }
  }
