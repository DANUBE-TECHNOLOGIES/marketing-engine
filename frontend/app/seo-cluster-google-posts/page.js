...
async function getPosts() {
  const res = await fetch("http://backend:4000/seo-cluster-google-posts", {
    cache: "no-store"
  });

  if (!res.ok) throw new Error("Erreur posts clusters SEO");

  return res.json();
}

export default async function SeoClusterGooglePostsPage() {
  ...
  return (
    <main className="min-h-screen bg-gray-100 p-8 text-gray-900">
      <div className="max-w-7xl mx-auto">
        <PageHeader
          title="Posts Google par cluster SEO"
          subtitle="Idées de Google Posts basées sur les intentions de recherche locales."
          action={
            <div className="flex gap-2">
              <ButtonLink href="/seo-keyword-clusters">Clusters SEO</ButtonLink>
              <ButtonLink href="/agency-google-posts-calendar">Calendrier posts</ButtonLink>
              <a href="http://localhost:4000/seo-cluster-google-posts/export" className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700">Export CSV</a>
