...
const { hydrateDestinationMediaAssets } = require("./destination-media-hydrator");
const { hydrateTeamMediaAssets } = require("./team-media-hydrator");
const { hydrateImageTextMediaAssets } = require("./image-text-media-hydrator");
const { hydrateGalleryMediaAssets } = require("./gallery-media-hydrator");
const { contentTargetsAgency } = require("../ai-content/editorial-targeting");
...
function isCanonicalDestinationBlock(block) {
  return ["destination-grid", "destinations", "destinations-highlight", "destination-recommendations"].includes(blockType(block));
}
...
async function enrichCanonicalDestinationBlocks({ database, tenantId, pages }) {
  ...
  const destinations = await database.destination.findMany({
    where: { tenantId: String(tenantId), status: "published", slug: { in: [...slugs] } },
    select: { id: true, name: true, slug: true, country: true, region: true, tagline: true, summary: true, heroImageUrl: true },
  });
        ...
        content: {
          ...
          items: items.map((item) => {
            ...
            return {
              ...
              ...(item.image || destination.heroImageUrl ? { image: item.image || destination.heroImageUrl } : {}),
...
function sendPublicSiteError(response, error) {
  ...
  response.status(Number.isInteger(statusCode) && statusCode >= 400 && statusCode <= 599 ? statusCode : 500).json({
    error: error?.code || "PUBLIC_SITE_READ_ERROR",
    message: statusCode >= 500 ? "Le mini-site est momentanément indisponible." : error?.message || "Impossible de charger le mini-site.",
    details: statusCode >= 500 ? {} : error?.details || {},
  });
}

async function resolvePublicTenantId(database, request) {
  const direct = String(request?.tenant?.id || request?.tenantId || request?.get?.("x-tenant-id") || "").trim();
...
async function filterAgencyInspirations({ database, tenantId, agencyId, pages }) {
  ...
  const contents = await database.seoContent.findMany({
    where: { tenantId: String(tenantId), id: { in: ids }, status: "published", publishedAt: { not: null } },
    select: { id: true, seo: true },
  });
  const allowed = new Set(contents.filter((content) => contentTargetsAgency(content, agencyId)).map((content) => String(content.id)));
  return pages.map((page) => ({
    ...page,
    blocks: (page.blocks || []).map((block) => {
      ...
      const next = { ...content };
      for (const key of ["items", "inspirations", "articles"]) {
        if (Array.isArray(content[key])) next[key] = content[key].filter((item) => allowed.has(String(item?.id)));
      }
...
async function hydrateContract({ database, contract }) {
  ...
  const hydratedPages = await hydratePublicDynamicBlocks({ prisma: database, tenantId, agencyId, pages: contract?.pages || [], includeUnpublishedBlocks: true });
  const destinationEnrichedPages = await enrichCanonicalDestinationBlocks({ database, tenantId, pages: hydratedPages });
  const destinationMediaPages = await hydrateDestinationMediaAssets({ prisma: database, tenantId, pages: destinationEnrichedPages });
  const teamMediaPages = await hydrateTeamMediaAssets({ prisma: database, tenantId, pages: destinationMediaPages });
  const imageTextMediaPages = await hydrateImageTextMediaAssets({ prisma: database, tenantId, pages: teamMediaPages });
  const galleryMediaPages = await hydrateGalleryMediaAssets({ prisma: database, tenantId, pages: imageTextMediaPages });
  const pages = await filterAgencyInspirations({ database, tenantId, agencyId, pages: galleryMediaPages });
  return { ...contract, pages, homePage: replacePageReference(contract?.homePage, pages), page: replacePageReference(contract?.page, pages) };
}
...
async function listPublishedAgencySites(database, tenantId) {
  ...
  return sites.map((site) => ({ slug: site.slug, name: site.name, city: site.agency?.city || null, publishedAt: site.publishedAt }));
}
...
function createPublicSiteReadRouter({ prisma } = {}) {
  ...
  router.get("/health", (request, response) => {
    response.json({ ok: true, capability: "public-site-read", version: "1.10", contentSource: "website-designer-v2-blocks", fallbackContentSource: "agency-site-sections", dynamicHydration: "single-pipeline", editorialTargeting: "tenant-and-agency-aware", publicTenantScope: "required", stableJsonErrors: true, writeOperations: false });
  });
  ...
  router.post("/sites/:siteSlug/preview-hydrate", async (request, response) => {
    try {
      const tenantId = await resolvePublicTenantId(database, request);
      const result = await hydratePreviewPage({ prisma: database, siteSlug: request.params.siteSlug, tenantId, page: request.body?.page || request.body || {} });
