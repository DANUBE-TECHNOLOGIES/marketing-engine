...
function routes({ prisma }) {
  ...
  router.get("/api/presence/providers", async (req, res) => {
    try {
      ...
      return res.json({ providers: listPresenceProviders().map((provider) => {
        const directory = directoryByProvider.get(provider.key);
        return { ...provider, readiness: getProviderReadiness(provider.key), legacyDirectory: directory ? { id: directory.id, name: directory.name, active: directory.active, priority: directory.priority, impactScore: directory.impactScore, difficulty: directory.difficulty } : null };
      }) });
  ...
  router.get("/api/presence/network/coverage", async (req, res) => {
    try {
      ...
      return res.json({ ok: true, ...buildNetworkCoverage(agencies, directories, listings) });
  ...
  router.get("/api/presence/agencies/:agencyId", async (req, res) => {
    try {
      ...
      if (!Number.isInteger(agencyId) || agencyId <= 0) return res.status(400).json({ error: "agencyId invalide" });
      const agency = await prisma.agency.findUnique({ where: { id: agencyId } });
      if (!agency) return res.status(404).json({ error: "Agence introuvable" });
      const listings = await prisma.directoryListing.findMany({ where: { agencyId }, include: { directory: true }, orderBy: { directoryId: "asc" } });
      return res.json({ agencyId, canonical: buildCanonicalAgencyIdentity(agency), listings: listings.map((listing) => ({ id: listing.id, status: listing.status, listingUrl: listing.listingUrl, lastCheckedAt: listing.lastCheckedAt, notes: listing.notes, directory: enrichDirectoryWithProvider(listing.directory) })) });
    } catch (error) { return res.status(500).json({ error: error.message }); }
  });
  ...
  router.post("/api/presence/google/sync", async (req, res) => {
    try {
      const schema = await auditDirectorySchema(prisma);
      if (!schema.ready) return res.status(503).json({ ok: false, error: "Schéma directories incomplet pour Presence", schema });
      const directory = await prisma.localDirectory.findUnique({ where: { name: "Google Business Profile" } });
      if (!directory) return res.status(409).json({ error: "Annuaire Google Business Profile non initialisé" });
      const agencies = await prisma.agency.findMany({ orderBy: { id: "asc" } });
      const results = [];
      for (const agency of agencies) {
        let listing = await prisma.directoryListing.findUnique({ where: { agencyId_directoryId: { agencyId: agency.id, directoryId: directory.id } } });
        if (!listing) listing = await prisma.directoryListing.create({ data: { agencyId: agency.id, directoryId: directory.id, status: "missing" } });
        try {
          const updated = await syncGoogleDirectoryListing(prisma, agency, listing);
          results.push({ agencyId: agency.id, agencyName: agency.name, status: updated.status, listingUrl: updated.listingUrl, notes: updated.notes, ok: true });
        } catch (error) {
          results.push({ agencyId: agency.id, agencyName: agency.name, ok: false, status: "error", error: error.message, googleStatus: error.status || null });
        }
      }
