...
function localContactIntro(site) {
  ...
  const area = nearby.length
    ? ` L’équipe accompagne également les voyageurs de ${joinCities(nearby)}.`
    : "";
    ...
    <section id="contact" className="public-site-section public-site-contact">
      <div className="public-site-container">
        ...
        <h2>
          {getSectionTitle(
            section,
            city ? `Contactez votre agence de voyages à ${city}` : "Contactez votre agence"
          )}
        </h2>
        ...
        <div className="public-site-agency-profile">
          <article className="public-site-agency-card">
            ...
            <div>
              ...
              {agency.address ? (
                ...
                <p>Adresse en cours de mise à jour.</p>
              )}
              {mapUrl ? (
                <a href={mapUrl} target="_blank" rel="noopener noreferrer">
                  {city ? `Itinéraire vers l’agence de ${city} →` : "Calculer l’itinéraire →"}
                </a>
              ) : null}
            </div>
          </article>
          ...
          <article className="public-site-agency-card">
            <span className="public-site-agency-icon">☎</span>
            <div>
              <small>{city ? `Téléphone de l’agence de ${city}` : "Téléphone"}</small>
              {agency.phone ? (
                <a className="public-site-agency-value" href={phoneHref(agency.phone)}>
                  {agency.phone}
                </a>
              ) : (
                <p>Numéro en cours de mise à jour.</p>
              )}
          ...
          <article className="public-site-agency-card">
            ...
            <div>
              ...
              <strong>Vous avez voyagé avec nous ?</strong>
              <p>
                {city
                  ? `Votre avis aide les voyageurs du secteur de ${city} à choisir leur agence de voyages.`
                  : "Votre avis aide les futurs voyageurs à choisir leur agence."}
              </p>
              {agency.googleReviewUrl ? (
                <a href={agency.googleReviewUrl} target="_blank" rel="noopener noreferrer">
                  {city ? `Déposer un avis Google sur l’agence de ${city} →` : "Déposer un avis Google →"}
                </a>
              ) : null}
            </div>
          </article>
        ...
        <div className="public-site-agency-actions">
          {agency.phone ? (
            <a className="public-site-button" href={phoneHref(agency.phone)}>
              {city ? `Appeler l’agence de ${city}` : "Appeler l’agence"}
            </a>
          ) : null}
          {agency.email ? (
            <a className="public-site-button public-site-button-outline" href={`mailto:${agency.email}`}>
              {city ? `Écrire à l’agence de ${city}` : "Envoyer un e-mail"}
            </a>
          ) : null}
        </div>
        ...
        <div
          ...
          aria-label={city ? `Continuer à préparer votre voyage avec l’agence de ${city}` : "Continuer à préparer votre voyage"}
        >
          <Link href={siteHref(site, "services")}>
            {city ? `Services de notre agence de voyages à ${city}` : "Découvrir nos services voyage"}
          </Link>
          <Link href={siteHref(site, "destinations")}>
            {city ? `Destinations conseillées par notre agence à ${city}` : "Explorer nos destinations"}
          </Link>
          <Link href={siteHref(site, "inspiration")}>
            {city ? `Conseils et inspirations voyage depuis ${city}` : "Lire nos inspirations voyage"}
          </Link>
        </div>
      </div>
    </section>
