...
function campaignRegenerationRoutes({ prisma }) {
  ...
  router.get("/api/presence/campaigns/:campaignId/regeneration-preview", async (req, res) => {
    try {
      ...
      const ready = compatibility.regenerationRequired === true && context.readiness?.ready === true;
      return res.status(ready ? 200 : 409).json({ ok: ready, externalWrite: false, persisted: false, sourceCampaignId: source.campaignId, compatibility, mode, readiness: context.readiness, preflightId: context.frozenPreflight?.preflightId || null });
  ...
  router.post("/api/presence/campaigns/:campaignId/regenerate", async (req, res) => {
    try {
      ...
      const approvedScope = {
        ...
        rolloutStage: mode.rolloutStage,
        sourceEvidenceCampaignId: srcScope.sourceEvidenceCampaignId || null,
        sourceEvidenceReportId: srcScope.sourceEvidenceReportId || null,
        sourceEvidenceReportCreatedAt: srcScope.sourceEvidenceReportCreatedAt || null,
        preflightRecoveryTrustFingerprint: trustFingerprint,
        regenerationOfCampaignId: source.campaignId,
        regenerationReason: regenerationReason(compatibility),
        regenerationSourceDecision: compatibility.decision
      };
      const approvedPlanFingerprint = stableId({ approvedScope, selected: context.plan.selected || [] });
      const campaign = await createCampaign(prisma, context.plan, req.body?.name || `Régénération ${source.name || source.campaignId}`, { pilot: true, preflightId: preflight.preflightId, approvedScope, approvedPlanFingerprint });
