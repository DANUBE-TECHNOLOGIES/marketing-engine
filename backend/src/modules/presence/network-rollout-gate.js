...
function stageDescriptor(evidence) {
  return Object.freeze({
    stagePercent: evidence.stagePercent,
    campaignId: evidence.campaign.campaignId,
    agencyCount: evidence.actualAgencyCount,
...
async function evaluateNetworkRolloutGate(prisma, totalAgencies) {
  ...
  if (!stage50) return Object.freeze({ ready: true, decision: "go", nextStagePercent: 50, maxAgencies: stageTargetAgencyCount(total, 50), blockers: Object.freeze([]), stages: Object.freeze([stageDescriptor(stage25)]) });

  const stage100 = await findStageEvidence(prisma, total, 100);
  if (!stage100) return Object.freeze({ ready: true, decision: "go", nextStagePercent: 100, maxAgencies: stageTargetAgencyCount(total, 100), blockers: Object.freeze([]), stages: Object.freeze([stageDescriptor(stage25), stageDescriptor(stage50)]) });

  return Object.freeze({ ready: true, decision: "complete", nextStagePercent: null, maxAgencies: total, blockers: Object.freeze([]), stages: Object.freeze([stageDescriptor(stage25), stageDescriptor(stage50), stageDescriptor(stage100)]) });
}
