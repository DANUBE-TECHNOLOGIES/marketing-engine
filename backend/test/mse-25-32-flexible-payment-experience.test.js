...
test("generic public copy never invents installment counts or no-fee claim", () => {
  const copy = buildPublicPaymentCopy({
    enabled: true,
    products: ["flight"],
  });

  assert.equal(copy.title, "Vos billets d’avion, payables en plusieurs fois");
...
test("planner targets home and flight pages, excludes drafts and duplicates", () => {
    ...
    site: {
      pages: [
        ...
        { slug: "circuits", status: "published", blocks: [] },
        {
          slug: "vols",
          status: "published",
          blocks: [{ blockType: "flexible_payment", content: {} }],
        },
  ...
  assert.equal(plan.version, "mse-25.32");
  assert.equal(plan.readOnly, true);
  assert.equal(plan.writes, false);
  assert.deepEqual(
    plan.proposals.map(({ slug, placement }) => ({ slug, placement })),
    [
