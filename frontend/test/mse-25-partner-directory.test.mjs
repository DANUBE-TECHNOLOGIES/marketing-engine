...
test("full partner directory exposes categorized supplier inventory with complete public editorial profiles", () => {
  ...
  for (const supplier of ["Catlante Catamarans", "CroisiEurope", "Rivages du Monde", "Ponant", "Hurtigruten", "MSC Croisières", "Costa Croisières", "FRAM", "TUI France", "Club Med", "Exotismes", "Jet tours", "Plein Vent", "Solea", "Travel Evasion", "Ôvoyages", "Asia", "Austral Lagons", "KUONI", "Salaün Holidays", "Ollandini", "Thalasso N°1"]) assert.match(catalogue, new RegExp(supplier.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
  for (const excluded of ["Worldia", "Aerosun", "Mega Vacances", "Visit Europe", "Planète Production"]) assert.doesNotMatch(catalogue, new RegExp(excluded, "i"));
  ...
  for (const id of ["double-sens", "destination-aventure", "la-francaise-des-circuits", "salaun-holidays", "pouchkine-tours", "nordiska", "top-of-travel", "voyages-internationaux", "rev-vacances"]) assert.match(circuitDetails, new RegExp(`(?:^|\\n)\\s*"?${id}"?\\s*:`));
  assert.doesNotMatch(circuitDetails, /visit-europe/);
  for (const id of ["fram", "tui-france", "club-med", "belambra", "boomerang", "exotismes", "jet-tours", "hotels-lagons", "lmx-voyages", "mondial-tourisme", "plein-vent", "solea", "pacha-tours", "heliades", "voyamar", "travel-evasion", "ovoyages"]) assert.match(stayDetails, new RegExp(`(?:^|\\n)\\s*"?${id}"?\\s*:`));
  for (const id of ["alma-latina", "australie-tours", "amerigo", "beachcomber-tours", "asia", "austral-lagons", "climats-du-monde", "jetset-voyages", "luxair-tours", "gaeland-ashling", "kuoni"]) assert.match(longHaulDetails, new RegExp(`(?:^|\\n)\\s*"?${id}"?\\s*:`));
  assert.doesNotMatch(longHaulDetails, /planete-production/);
  ...
  assert.match(catalogue, /\/partners\/manual\/ovoyages\.webp/);
