...
function createPreflightId(readiness) {
  const payload = JSON.stringify({ generatedAt: readiness.generatedAt, migrations: readiness.migrations, catalog: readiness.catalog?.summary, network: readiness.network, operational: { readyForGoogleApi: readiness.operational?.readyForGoogleApi, googleWritesEnabled: readiness.operational?.googleWritesEnabled }, pilot: readiness.pilot });
...
module.exports = { createPreflightId, freezeDeploymentPreflight, listDeploymentPreflights, getLatestDeploymentPreflight };
