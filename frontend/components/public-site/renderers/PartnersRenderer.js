...
const DIRECTORY_TYPES = new Set(["partner-directory", "partners-directory", "partner-categories"]);
const PARTNER_LOGO_WIDTH = 180;
...
function pageHasExplicitDirectory(page) {
  const sections = Array.isArray(page?.sections) ? page.sections : Array.isArray(page?.blocks) ? page.blocks : [];
  return sections.some((candidate) => DIRECTORY_TYPES.has(getSectionType(candidate)));
}
  ...
  return (
    <div className={`${styles.networkGrid} public-site-partners-grid public-site-partners-grid--network`}>
      {items.map((item, index) => {
        ...
        return (
          <div key={item.id || name || index} className={`${styles.networkCard} ${isTui ? styles.tuiCard : ""} public-site-partner-card`} data-partner-id={item.id || undefined}>
            {isTui ? (
              <>
                <img src={safePartnerAssetUrl(item.logoUrl)} alt={item.alt || "Logo TUI"} loading="lazy" decoding="async" width={PARTNER_LOGO_WIDTH} height={PARTNER_LOGO_HEIGHT} className={styles.tuiMainLogo} />
                <div className={styles.tuiChildren}>
                  {(item.children || []).map((child) => (
                    <div key={child.id} className={styles.tuiChild}>
                      <img src={safePartnerAssetUrl(child.logoUrl)} alt={`Logo ${child.name}`} loading="lazy" decoding="async" width={PARTNER_LOGO_WIDTH} height={PARTNER_LOGO_HEIGHT} className={styles.tuiChildLogo} />
                    </div>
                  ))}
                </div>
              </>
            ) : logo ? (
              <img src={logo} alt={item.alt || `Logo ${name}`} loading="lazy" decoding="async" width={PARTNER_LOGO_WIDTH} height={PARTNER_LOGO_HEIGHT} className={styles.networkLogo} />
            ) : <strong>{name}</strong>}
          </div>
        );
      })}
    ...
    <div className={`public-site-agency-partners ${styles.agencyWrap}`}>
      ...
      <div className={`public-site-agency-partners-grid ${styles.agencyGrid}`}>
        {items.map((item, index) => {
          ...
          const mark = item.logoUrl ? <img src={item.logoUrl} alt={item.alt || `Logo ${name}`} loading="lazy" decoding="async" width={PARTNER_LOGO_WIDTH} height={PARTNER_LOGO_HEIGHT} className={styles.agencyLogo} /> : <strong>{name}</strong>;
...
export { PARTNER_LOGO_HEIGHT, PARTNER_LOGO_WIDTH, pageHasExplicitDirectory, pageIsHome };
