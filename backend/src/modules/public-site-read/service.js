...
function normalizeSlug(value) {
  ...
  if (
    ...
    const error =
      new Error(
        "Slug de mini-site invalide."
      );

    error.code =
      "PUBLIC_SITE_SLUG_INVALID";
...
class PublicSiteReadService {
  ...
  buildSelect() {
    const siteFields =
      fieldsFor("AgencySite");

    const pageFields =
      fieldsFor("AgencySitePage");

    const blockModelName =
  ...
  async bySlug(siteSlug) {
    ...
    /*
     * Contrat public strict :
     * un mini-site brouillon ou dépublié n'est jamais exposé.
     */
    ...
    /*
     * Important :
     * aucune page brouillon ne doit apparaître publiquement,
     * même lorsqu'un site ne possède encore aucune page publiée.
     */
    const visiblePages =
      publishedPages;
