...
test("full partner directory exposes the network categories and supplier inventory", () => {
  ...
  for (const enrichedId of ["top-of-travel", "visit-europe", "voyages-internationaux", "boomerang", "mondial-tourisme", "la-francaise-des-circuits", "pacha-tours", "kuoni", "ollandini", "mmv"]) assert.match(details, new RegExp(`(?:^|\\n)\\s*"?${enrichedId}"?\\s*:`));
  ...
  assert.match(profile, /getResolvedPartnerDetails/); assert.match(profile, /getPublishablePartnerProfiles/); assert.match(profile, /publishable:\s*verification\.status !== "identity-review"/); assert.match(profile, /visibleTags:\s*Array\.isArray\(partner\.tags\) \? partner\.tags\.slice\(0, 2\)/);

  assert.match(catalogue, /\/partners\/kuoni-official\.webp/);
  for (const term of [/Top Clubs/, /Kappa Club/, /Mondi Club/, /Voyage en train/, /Circuit accompagné/, /Circuit privatif/, /Méditerranée orientale/, /Alpes françaises/]) assert.match(details, term);
