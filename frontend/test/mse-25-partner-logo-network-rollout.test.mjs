...
test("network rollout never auto-ingests permission-held partner logos", () => {
  const rollout = read("scripts/partner-logo-network-rollout.mjs");
  const backlog = read("components/page-builder/shared/partnerLogoBacklog.js");

  for (const id of ["ponant", "celestyal-cruises", "cfc", "salaun-holidays", "nordiska", "pouchkine-tours", "belambra", "mondial-tourisme", "plein-vent", "heliades", "voyamar"]) {
    assert.match(backlog, new RegExp(`${id}[\\s\\S]*state: "permission-required"`));
  }
