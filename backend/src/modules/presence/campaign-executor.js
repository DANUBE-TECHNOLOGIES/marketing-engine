...
async function executeFrozenCampaign(prisma, campaignId, options = {}) {
  ...
  for (const item of items) {
    ...
    if (item.providerKey !== "google_business_profile" || item.remediationKind !== "managed_api") {
      ...
      results.push({ item, status: "skipped", reason: "provider_not_supported_for_campaign_execution" }); continue;
    }
    if (!item.agencyId || !Array.isArray(item.drift) || !item.drift.length) {
      await upsertCampaignExecution(prisma, { campaignId, campaignIndex: item.campaignIndex, providerKey: item.providerKey, agencyId: item.agencyId, status: "skipped" });
      results.push({ item, status: "skipped", reason: "invalid_frozen_item" }); continue;
    }
    ...
    if (!agency) {
      ...
      results.push({ item, status: "failed", error: "Agence introuvable" }); continue;
    }
    ...
    if (preview.risk.requiresSensitiveConfirmation && options.confirmSensitive !== true) {
      ...
      results.push({ item, status: "blocked_sensitive", risk: preview.risk }); continue;
    }
  ...
  const summary = { planned: items.length, submitted: results.filter((r) => r.status === "submitted").length, alreadyProcessed: results.filter((r) => r.status === "already_processed").length, skipped: results.filter((r) => r.status === "skipped").length, blockedSensitive: results.filter((r) => r.status === "blocked_sensitive").length, failed: results.filter((r) => r.status === "failed").length };
  if ((summary.submitted > 0 || summary.alreadyProcessed === items.length) && summary.failed === 0 && summary.blockedSensitive === 0) await transitionCampaign(prisma, campaignId, "verifying", { reason: "campaign_execution_submitted", payload: summary });
