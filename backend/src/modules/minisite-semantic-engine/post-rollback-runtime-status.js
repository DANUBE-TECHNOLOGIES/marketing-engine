...
function buildPostRollbackRuntimeStatus({ env = process.env } = {}) {
  ...
  const rollbackReportPath =
    env.MSE_25_68_ROLLBACK_REPORT || latestJson(rollbackDir, "mse-25-67-guarded-rollback-");
  const verificationReportPath =
    env.MSE_25_68_VERIFICATION_REPORT ||
    latestJson(verificationDir, "mse-25-68-post-rollback-verification-");
  ...
  const rollbackCertified = Boolean(
    audit &&
      audit.type === "MSE_25_67_GUARDED_ROLLBACK_AUDIT" &&
      audit.transactional === true &&
      audit.automaticRollback === false &&
      audit.pageCreated === false &&
      audit.published === false
  );

  const verificationCertified = Boolean(
    verification &&
      verification.type === "MSE_25_68_POST_ROLLBACK_VERIFICATION_REPORT" &&
      verification.certified === true &&
      certification?.certified === true &&
      verification.readOnly === true &&
      verification.writes === false &&
      verification.publicWrites === false
  );
  ...
  if (readError) {
    ...
    state = verification?.verification?.manualInterventionRequired
      ? "POST_ROLLBACK_MANUAL_INTERVENTION_REQUIRED"
      : "POST_ROLLBACK_VERIFIED";
    reason = verification?.verification?.manualInterventionRequired
      ? "VERIFICATION_REQUIRES_MANUAL_INTERVENTION"
      : "CERTIFIED_POST_ROLLBACK_VERIFICATION_AVAILABLE";
  }
  ...
  return {
    ...
    publicWrites: false,
    rollback: {
      available: Boolean(rollbackReportPath),
      certified: rollbackCertified,
      reportPath: rollbackReportPath,
      pageIdentity: audit?.pageIdentity || null,
      siteSlug: audit?.siteSlug || null,
      page: audit?.page || null,
    },
    verification: {
      available: Boolean(verificationReportPath),
      certified: verificationCertified,
      ...
      manualInterventionRequired:
        verification?.verification?.manualInterventionRequired ?? null,
