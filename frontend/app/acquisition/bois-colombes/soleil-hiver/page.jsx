...
const destinationIdeas = {
  plage: ["Océan Indien", "Caraïbes", "Cap-Vert"], circuit: ["Asie", "Amérique latine", "Afrique australe"], safari: ["Tanzanie", "Kenya", "Afrique du Sud"], croisiere: ["Caraïbes", "Océan Indien", "Méditerranée"], combine: ["Île Maurice & Dubaï", "Sri Lanka & Maldives", "Safari & Zanzibar"], "a-decouvrir": ["Océan Indien", "Caraïbes", "Afrique"],
};
...
export default function Funnel() {
  ...
  async function submit(e) {
    ...
    } catch (err) { setResult({ error: err.message }); setState("error"); }
  }
  ...
  if (state === "done") return (
    <main style={S.main}><div style={S.shell}><Brand /><section style={S.resultCard}>
      ...
      {result?.lead?.duplicate && <p style={S.mini}>Nous avions déjà reçu cette demande récemment : elle n'a pas été créée une seconde fois.</p>}<p style={S.privacy}>Vos préférences de contact sont respectées. Aucun rappel téléphonique sans votre accord explicite.</p>
    </section></div></main>
  );
...
function Brand({ light = false }) {
  return <header style={S.brand}><div><div style={S.logoCrop}><img src="/brand/logo-mondescale.png" alt="Mondescale Voyages" style={S.logoImage} /></div><span style={{ ...S.brandLocation, color: light ? "rgba(255,255,255,.68)" : "#61736d" }}>Bois-Colombes</span></div></header>;
}
...
const S = {
  ...
  brand: { display: "flex", alignItems: "center", minHeight: 78 }, logoCrop: { width: 194, height: 70, overflow: "hidden", borderRadius: 11, background: "#fff", position: "relative" }, logoImage: { position: "absolute", width: 286, height: 160, objectFit: "cover", left: "50%", top: "50%", transform: "translate(-50%,-50%)" }, brandLocation: { display: "block", marginTop: 6, fontSize: 9, fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase" },
