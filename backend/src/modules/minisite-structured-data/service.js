...
const { attachLocalIntentTargetQualityReadiness } = require("./local-intent-target-quality-readiness");
...
class MiniSiteStructuredDataService {
  ...
  health() { return { status: "ok", capability: "minisite-structured-data", persistence: false, destructive: false, deterministic: true, tenantScoped: true, publicGraphPublishedOnly: true, destinationSitemap: "localized-per-published-agency-site", editorialSitemap: "canonical-agency-only", inspirationIndexSitemap: "only-when-public-content-targets-agency", contentQualityIndexability: "critically-thin-nonfunctional-pages-excluded", crawlabilityAudit: "discovery-source-and-orphan-detection", indexationReadiness: "technical-local-seo-content-uniqueness-semantic-depth-search-intent-concrete-target-and-target-quality-safe-to-submit-report", localSeoCoverage: "published-content-h1-nap-structured-data-linking-geographic-semantic-depth-search-intent-target-mapping-and-target-quality-audit", localIntentTargetMapping: "same-page-city-and-commercial-intent-mapping", localIntentTargetQuality: "target-title-meta-h1-body-and-depth-quality", localContentUniqueness: "cross-agency-published-homepage-similarity-audit", xmlSitemap: "network-and-site-candidate-rendering", publicOrigin: this.publicOrigin, schemas: ["TravelAgency", "LocalBusiness", "WebSite", "WebPage", "BreadcrumbList", "FAQPage"], operations: ["previewNetwork", "previewSitemap", "previewSite", "networkSitemapXml", "siteSitemapCandidate", "localSeoCoverage", "localIntentTargetMapping", "localContentUniqueness"] }; }
  ...
  async previewSitemap({ tenantId } = {}) {
    ...
    const targetQualityReady = attachLocalIntentTargetQualityReadiness(targetReady, coverage);
    const uniqueness = auditLocalContentUniqueness(publicSites);
    return attachLocalContentReadiness(targetQualityReady, uniqueness);
  }
