...
class MiniSiteStructuredDataService {
  ...
  health() { return { status: "ok", capability: "minisite-structured-data", persistence: false, destructive: false, deterministic: true, tenantScoped: true, publicGraphPublishedOnly: true, destinationSitemap: "localized-per-published-agency-site", editorialSitemap: "canonical-agency-only", inspirationIndexSitemap: "only-when-public-content-targets-agency", contentQualityIndexability: "critically-thin-nonfunctional-pages-excluded", crawlabilityAudit: "discovery-source-and-orphan-detection", indexationReadiness: "per-site-safe-to-submit-report", localSeoCoverage: "published-content-and-geographic-gap-audit", xmlSitemap: "network-and-site-candidate-rendering", publicOrigin: this.publicOrigin, schemas: ["TravelAgency", "LocalBusiness", "WebSite", "WebPage", "BreadcrumbList", "FAQPage"], operations: ["previewNetwork", "previewSitemap", "previewSite", "networkSitemapXml", "siteSitemapCandidate", "localSeoCoverage"] }; }
  ...
  async previewSitemap({ tenantId } = {}) { const [sites, inspirations, destinations] = await Promise.all([this.repository.listSites(tenantId), this.repository.listPublishedEditorialContents(tenantId), this.repository.listPublishedDestinations(tenantId)]); const sitemap = buildPublicSitemap({ sites, inspirations, destinations, publicOrigin: this.publicOrigin }); return attachIndexationReadiness(auditSitemapCrawlability(applyContentQualityIndexabilityContract(applyInspirationIndexabilityContract(sitemap, sites, inspirations), sites))); }
  ...
  async localSeoCoverage({ tenantId, contexts = {} } = {}) { const sites = (await this.repository.listSites(tenantId)).map(publicStructuredDataSite).filter(Boolean); return auditLocalSeoCoverage(sites, contexts); }
