...
function frozenNetworkTrustClean(frozen){const trust=frozen?.report?.pilotEvidence?.networkRecoveryTrust;return Boolean(trust)&&Number(trust.critical||0)===0;}
...
async function evidenceForCampaign(prisma, campaign) {
  ...
  if (!frozenNetworkTrustClean(frozen)) return null;
...
async function evaluateNetworkRolloutGate(prisma,totalAgencies){const total=Math.max(0,Number(totalAgencies||0));if(!total)return Object.freeze({ready:false,decision:"no_go",nextStagePercent:25,blockers:Object.freeze(["network_agencies_missing"]),stages:Object.freeze([])});const recoveryTrust=await buildRecoveryTrustOverview(prisma,200);if(Number(recoveryTrust?.summary?.critical||0)>0)return Object.freeze({ready:false,decision:"no_go",nextStagePercent:null,maxAgencies:0,blockers:Object.freeze(["critical_recovery_trust_blocks_rollout",...(recoveryTrust.campaigns||[]).filter(x=>x.severity==="critical").map(x=>`critical_recovery:${x.campaignId}`)]),stages:Object.freeze([]),recoveryTrust});const stage25=await findStageEvidence(prisma,total,25);if(!stage25)return Object.freeze({ready:false,decision:"no_go",nextStagePercent:25,blockers:Object.freeze(["rollout_25_evidence_missing"]),stages:Object.freeze([]),recoveryTrust});const stage50=await findStageEvidence(prisma,total,50);if(!stage50)return Object.freeze({ready:true,decision:"go",nextStagePercent:50,maxAgencies:stageTargetAgencyCount(total,50),blockers:Object.freeze([]),stages:Object.freeze([stageDescriptor(stage25)]),recoveryTrust});const stage100=await findStageEvidence(prisma,total,100);if(!stage100)return Object.freeze({ready:true,decision:"go",nextStagePercent:100,maxAgencies:stageTargetAgencyCount(total,100),blockers:Object.freeze([]),stages:Object.freeze([stageDescriptor(stage25),stageDescriptor(stage50)]),recoveryTrust});return Object.freeze({ready:true,decision:"complete",nextStagePercent:null,maxAgencies:total,blockers:Object.freeze([]),stages:Object.freeze([stageDescriptor(stage25),stageDescriptor(stage50),stageDescriptor(stage100)]),recoveryTrust});}
module.exports={ROLLOUT_STAGES,stageTargetAgencyCount,campaignAgencyCount,campaignStage,frozenNetworkTrustClean,recoveryEvidenceStillValid,evidenceForCampaign,findStageEvidence,evaluateNetworkRolloutGate};
