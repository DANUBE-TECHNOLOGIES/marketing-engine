...
function extractCandidates(html, baseUrl, partner) {
  ...
  while ((match = tagRegex.exec(html))) {
    const tag = match[0];
    const context = [tag.match(/(?:alt|title|class|id|rel|property|name)=["']([^"']+)["']/i)?.[1], tag].filter(Boolean).join(" ");
...
async function fetchText(url) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: { "user-agent": "Mozilla/5.0 (compatible; MondescalePartnerAssetAudit/2.0)", accept: "text/html,application/xhtml+xml" },
    });
...
async function probeAsset(url) {
  ...
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const response = await fetch(decodeHtml(url), {
      method: "GET",
      redirect: "follow",
      signal: controller.signal,
      headers: { "user-agent": "Mozilla/5.0 (compatible; MondescalePartnerAssetAudit/2.0)", range: "bytes=0-2047" },
    });
...
const selected = catalogue.filter((partner) => {
  if (categoryArg && partner.category !== categoryArg) return false;
  if (partnerArg && partner.id !== partnerArg) return false;
  const verification = getPartnerVerification(partner.id);
  if (verification.status === "identity-review" || verification.status === "catalogue-excluded" || verification.status === "asset-permission-review") return false;
  if (String(partner.logoUrl || "").trim()) return false;
  const source = registries.get(partner.category)?.[partner.id] || null;
...
for (const partner of selected) {
  const source = registries.get(partner.category)?.[partner.id] || null;
  const sourcePage = source?.sourcePage || source?.sourceUrl || source?.preferredSource || "";
  if (!sourcePage) {
    rows.push({ id: partner.id, name: partner.name, category: partner.category, sourceStatus: source?.status || null, page: { ok: false, error: "missing source page" }, candidates: [] });
    continue;
  }
  ...
  rows.push({
    ...
    sourcePage,
    page: { ok: page.ok, status: page.status, finalUrl: page.finalUrl, contentType: page.contentType, error: page.error || null },
    candidates: probed,
  });
}
...
console.log(JSON.stringify({
  ...
  filters: { category: categoryArg || null, partner: partnerArg || null },
  selectedPartners: selected.length,
  acceptedAssetFormats: ["svg", "webp", "png"],
  partners: rows,
}, null, 2));
