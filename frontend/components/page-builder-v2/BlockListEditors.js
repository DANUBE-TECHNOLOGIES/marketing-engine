  ...
  return (
    <div className={styles.listEditor}>
      {safeItems.map((item, index) => (
        <section className={styles.listEditorItem} key={item.id || `${index}`}>
          <ItemToolbar index={index} count={safeItems.length} onMove={(direction) => move(index, direction)} onDelete={() => deleteItem(index)} />
          {children({ item, index, update: (value) => updateItem(index, value) })}
        </section>
      ))}
      {canAdd ? (
        <button type="button" className={styles.addListItem} onClick={() => onChange([...safeItems, createItem()])}>
          + {addLabel}
        </button>
      ) : null}
    </div>
...
export function PartnerLogosEditor({ networkItems, agencyPartners, maxAgencyPartners = 3, onChange }) {
  ...
  return (
    <div className={styles.partnerEditor}>
      <div className={styles.partnerLockedPanel}>
        <strong>Socle réseau Mondescale</strong>
        <p>Ces 7 partenaires sont communs à tous les mini-sites et ne peuvent pas être modifiés ici.</p>
        <div className={styles.partnerLockedList}>
          {locked.map((item) => <span key={item.id || item.name}>{item.name || item.title}</span>)}
        </div>
      </div>

      <div className={styles.partnerAgencyHeader}>
        <strong>Partenaires de l’agence</strong>
        <small>{agency.length}/{maxAgencyPartners} emplacement{maxAgencyPartners > 1 ? "s" : ""}</small>
      </div>
...
export function FaqEditor({ items, onChange }) {
  return <ListEditor items={items} onChange={onChange} addLabel="Ajouter une question" createItem={() => ({ question: "Nouvelle question", answer: "Nouvelle réponse" })}>{({ item, update }) => <><Field label="Question" value={item.question} onChange={(question) => update({ ...item, question })} /><Field label="Réponse" value={item.answer} multiline onChange={(answer) => update({ ...item, answer })} /></>}</ListEditor>;
}

export function FeaturesEditor({ items, onChange }) {
  return <ListEditor items={items} onChange={onChange} addLabel="Ajouter un point fort" createItem={() => ({ icon: "✦", title: "Nouveau point fort", text: "Description du point fort." })}>{({ item, update }) => <><Field label="Icône" value={item.icon} onChange={(icon) => update({ ...item, icon })} /><Field label="Titre" value={item.title} onChange={(title) => update({ ...item, title })} /><Field label="Description" value={item.text} multiline onChange={(text) => update({ ...item, text })} /></>}</ListEditor>;
}

export function GalleryEditor({ images, onChange, assets = [], loading = false }) {
  return <ListEditor items={images} onChange={onChange} addLabel="Ajouter une image" createItem={() => ({ imageAssetId: "", url: "", alt: "", caption: "" })}>{({ item, update }) => { const selectedAsset = item.imageAssetId ? assets.find((asset) => asset.id === item.imageAssetId) || null : null; const previewUrl = item.url || selectedAsset?.url || ""; return <>{previewUrl ? <img className={styles.editorThumbnail} src={previewUrl} alt={item.alt || ""} /> : null}<MediaPicker assets={assets} loading={loading} selectedAssetId={item.imageAssetId || ""} onSelect={(asset) => update({ ...item, imageAssetId: asset.id, url: null, alt: item.alt || asset.altText || "" })} onClear={() => update({ ...item, imageAssetId: "" })} /><details><summary>URL d’image héritée</summary><Field label="URL de l’image" value={item.url} onChange={(url) => update({ ...item, url })} /></details><Field label="Texte alternatif" value={item.alt} onChange={(alt) => update({ ...item, alt })} /><Field label="Légende" value={item.caption} multiline onChange={(caption) => update({ ...item, caption })} /></>; }}</ListEditor>;
}

export function TestimonialsEditor({ items, onChange }) {
  return <ListEditor items={items} onChange={onChange} addLabel="Ajouter un témoignage" createItem={() => ({ author: "Client", text: "Un excellent accompagnement.", rating: 5 })}>{({ item, update }) => <><Field label="Auteur" value={item.author} onChange={(author) => update({ ...item, author })} /><Field label="Témoignage" value={item.text} multiline onChange={(text) => update({ ...item, text })} /><Field label="Note" type="number" value={item.rating} onChange={(rating) => update({ ...item, rating: Math.max(1, Math.min(5, Number(rating) || 1)) })} /></>}</ListEditor>;
}

export function TeamEditor({ members, onChange, assets = [], loading = false }) {
  return <ListEditor items={members} onChange={onChange} addLabel="Ajouter un membre" createItem={() => ({ id: `team-${Date.now()}`, name: "", role: "Conseiller voyage", imageAssetId: "", imageUrl: "", imageAlt: "", bio: "" })}>{({ item, update }) => { const selectedAsset = item.imageAssetId ? assets.find((asset) => asset.id === item.imageAssetId) || null : null; const previewUrl = item.imageUrl || selectedAsset?.url || ""; return <>{previewUrl ? <img className={styles.editorThumbnail} src={previewUrl} alt={item.imageAlt || item.name || "Membre de l'équipe"} /> : null}<Field label="Nom" value={item.name} onChange={(name) => update({ ...item, name })} /><Field label="Fonction" value={item.role} onChange={(role) => update({ ...item, role })} /><MediaPicker assets={assets} loading={loading} selectedAssetId={item.imageAssetId || ""} onSelect={(asset) => update({ ...item, imageAssetId: asset.id, imageAlt: item.imageAlt || asset.altText || (item.name ? `Portrait de ${item.name}` : "") })} onClear={() => { const { imageAssetId: _a, imageUrl: _b, __mediaSource: _c, __mediaVersion: _d, ...rest } = item; update(rest); }} /><Field label="Texte alternatif de la photo" value={item.imageAlt} onChange={(imageAlt) => update({ ...item, imageAlt })} /><details><summary>URL de photo héritée</summary><Field label="URL de la photo" value={item.imageUrl} onChange={(imageUrl) => update({ ...item, imageUrl })} /></details><Field label="Présentation" value={item.bio} multiline onChange={(bio) => update({ ...item, bio })} /></>; }}</ListEditor>;
}

export function StringListEditor({ items, onChange, label, addLabel }) {
  return <ListEditor items={(items || []).map((value) => ({ value }))} onChange={(nextItems) => onChange(nextItems.map((item) => item.value))} addLabel={addLabel} createItem={() => ({ value: "" })}>{({ item, update }) => <Field label={label} value={item.value} onChange={(value) => update({ value })} />}</ListEditor>;
}
