...
function evaluatePresenceStorage(rows = []) {
  ...
  return Object.freeze({ ready: missing.length === 0, required: REQUIRED_TABLES, present: Object.freeze([...found]), missing: Object.freeze(missing) });
}
...
async function auditPresenceStorage(prisma) {
  const rows = await prisma.$queryRawUnsafe(`
    ...
    WHERE table_schema = current_schema()
      AND table_name IN ('PresenceOperationAudit', 'PresenceOperationSnapshot', 'PresenceCampaign', 'PresenceCampaignEvent', 'PresenceCampaignExecution', 'PresenceCampaignReport', 'PresenceCitationObservation', 'PresenceDeploymentPreflight')
    ORDER BY table_name
  `);
