...
function normalizeColor(value, fallback) {
  ...
  return validHexColor(candidate) ? candidate.toLowerCase() : fallback;
}
...
function normalizeBrandIdentity(payload) {
  ...
  const values = source.values || source.theme || source.identity || source;

  return {
    ...source,
    primaryColor: normalizeColor(
      values.primaryColor || values.primary || values.colors?.primary,
      DEFAULT_IDENTITY.primaryColor
    ),
    secondaryColor: normalizeColor(
      values.secondaryColor || values.secondary || values.colors?.secondary,
      DEFAULT_IDENTITY.secondaryColor
    ),
    accentColor: normalizeColor(
      values.accentColor || values.accent || values.colors?.accent,
      DEFAULT_IDENTITY.accentColor
    ),
    backgroundColor: normalizeColor(
      values.backgroundColor || values.background || values.colors?.background,
      DEFAULT_IDENTITY.backgroundColor
    ),
    textColor: normalizeColor(
      values.textColor || values.text || values.colors?.text,
...
async function fetchBrandIdentity(agencyId) {
  ...
  for (const url of candidates) {
    ...
    return normalizeBrandIdentity(await parseJsonResponse(response));
  }
...
function buildBrandProfilePayload({ agencyId, profile, identity }) {
  const normalizedAgencyId = normalizeAgencyId(agencyId);
  const values = {
    ...(profile.values || {}),
...
async function saveBrandIdentity({ agencyId, profile, identity }) {
  const payload = buildBrandProfilePayload({ agencyId, profile, identity });
  const normalizedAgencyId = normalizeAgencyId(agencyId);
  const candidates = [
    {
      url: `/api/brand-profile/agencies/${normalizedAgencyId}`,
      method: "PUT",
    },
    {
      url: `/api/brand-profile?agencyId=${normalizedAgencyId}`,
      method: "PUT",
    },
  ];
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      return await fetchBrandIdentity(normalizedAgencyId);
    } catch (error) {
      lastError = error;
  ...
  throw (
    lastError ||
    new Error("Aucun contrat d’enregistrement du Brand Profile n’est disponible.")
  );
}

function validateBrandIdentity(identity) {
  const errors = {};
  ...
  for (const field of [
    ...
    if (!validHexColor(identity[field])) {
      errors[field] = "La couleur doit utiliser le format #RRGGBB.";
    }
  }

  if (!String(identity.headingFont || "").trim()) {
    errors.headingFont = "La police des titres est obligatoire.";
  }

  if (!String(identity.bodyFont || "").trim()) {
    errors.bodyFont = "La police du texte est obligatoire.";
  }
