...
async function tenantIdForDestinationRequest(prisma, req) {
  const direct = String(
  ...
  if (!tenant) {
    ...
    error.code = 'DESTINATION_TENANT_NOT_FOUND';
    throw error;
  }
...
module.exports = ({ prisma }) => {
  ...
  router.get('/destinations', async (req, res, next) => {
    try {
      const tenantId = await tenantIdForDestinationRequest(prisma, req);
      res.json(await service.list(tenantId, req.query.status === 'published'));
    } catch (e) { next(e); }
  });
  ...
  router.get('/destinations/:slug', async (req, res, next) => {
    try {
      const tenantId = await tenantIdForDestinationRequest(prisma, req);
      res.json(await service.get(tenantId, req.params.slug));
    } catch (e) { next(e); }
  });

  router.post('/destinations/seed/budapest', async (req, res, next) => {
    try {
      const tenantId = await tenantIdForDestinationRequest(prisma, req);
      res.status(201).json(await service.seedBudapest(tenantId));
    } catch (e) { next(e); }
  });
...
module.exports.tenantIdForDestinationRequest = tenantIdForDestinationRequest;
module.exports.publicTenantId = tenantIdForDestinationRequest;
