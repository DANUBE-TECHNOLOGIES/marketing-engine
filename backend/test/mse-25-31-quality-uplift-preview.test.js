...
class FakeService {
  constructor() {
    this.repository = { listSites: async () => [{ agencyId: 1, slug: "published-site", status: "published" }, { agencyId: 2, slug: "draft-site", status: "draft" }] };
  }
  async buildAgencyContentOptimization({ agencyId }) {
    assert.equal(Number(agencyId), 1);
    return {
      agencyId: 1, siteSlug: "published-site", city: "Gien", excludedPages: [{ slug: "contact", reason: "canonical-route-managed" }],
      pages: [{ slug: "home", title: "Accueil", published: true, page: { slug: "home", title: "Accueil", status: "published", published: true, seoTitle: "Agence de voyages à Gien", metaDescription: "Agence de voyages à Gien." }, currentBlocks: [{ id: "hero-home", blockType: "hero", content: { title: "Agence de voyages à Gien" } }, { blockType: "rich_text", content: { html: "Conseils voyage à Gien." } }] }],
    };
  }
}
...
test("siteFromAgencyPlan exposes persisted blocks to the quality planner", () => {
  const site = siteFromAgencyPlan({ agencyId: 7, siteSlug: "test-site", city: "Nevers", pages: [{ slug: "services", published: true, page: { slug: "services", status: "published" }, currentBlocks: [{ blockType: "rich_text", content: { html: "Texte réel" } }] }] });
...
test("exact metadata operations seal deterministic write targets, source fingerprints and final values", () => {
  const site = { agency: { id: 1, name: "Mondescale Gien", city: "Gien" } };
  const page = { slug: "croisieres", title: "Croisières", seoTitle: "Croisières", metaDescription: "Découvrez nos croisières.", blocks: [{ id: "hero-cruises", blockType: "hero", content: { title: "Croisières" } }, { id: "copy-cruises", blockType: "rich_text", content: { html: "Texte" } }] };
  ...
  assert.deepEqual(h1.target, { scope: "block", blockType: "hero", blockId: "hero-cruises", field: "title" });
...
test("H1 sealing is fail-closed when the hero target is ambiguous or has no exact id", () => {
  ...
  assert.equal(exactHeroTarget({ blocks: [{ id: "hero-a", blockType: "hero", content: {} }, { id: "hero-b", blockType: "hero", content: {} }] }), null);
  const target = exactHeroTarget({ blocks: [{ blockType: "hero", content: {} }] });
  assert.deepEqual(target, { scope: "block", blockType: "hero", blockId: null, field: "title" });
  const sealed = sealOperationFinalValue({ type: "strengthen-h1" }, { agency: { city: "Gien" } }, { slug: "home", blocks: [{ blockType: "hero", content: { title: "Accueil" } }] });
...
test("internal-link sealing binds the exact persisted source block and source HTML", () => {
  ...
  const site = { agencyId: 4, agency: { id: 4, city: "Gien" }, pages: [{ slug: "home", title: "Accueil", blocks: [{ id: "copy-home", blockType: "rich_text", content: { html: sourceHtml } }] }, { slug: "avis", title: "Avis clients", blocks: [{ id: "copy-avis", blockType: "rich_text", content: { html: "<p>Vos avis.</p>" } }] }] };
  const proposal = { pageSlug: "avis", diagnostics: { internalLink: { path: "/agence/gien/avis" } } };
  const operation = sealInternalLinkOperation({ type: "add-internal-link", suggestedSourceSlugs: ["home"] }, proposal, site, site.pages[1]);
  const expectedLabel = internalLinkLabel(site.pages[1], site);
  assert.deepEqual(operation.target, { scope: "block", pageSlug: "home", blockType: "rich_text", blockId: "copy-home", field: "content.html" });
...
test("Budapest and commitments anchors remain natural and purpose-specific", () => {
  ...
  const cases = [
    [{ slug: "budapest", title: "Voyage à Budapest" }, /Budapest/i],
    [{ slug: "budapest-best-time", title: "Quand partir à Budapest ?" }, /partir|période|dates/i],
    [{ slug: "budapest-faq", title: "Questions fréquentes sur Budapest" }, /FAQ|questions|réponses/i],
    [{ slug: "budapest-where-stay", title: "Où dormir à Budapest ?" }, /dormir|séjourner|hébergement/i],
    [{ slug: "engagements", title: "Nos engagements" }, /nos engagements|engagements de notre agence/i],
  ];
  for (const [page, expected] of cases) {
    const label = internalLinkLabel(page, site);
    assert.match(label, expected);
    assert.doesNotMatch(label, /^Découvrir (Voyage|Quand partir|Questions fréquentes|Nos engagements)/i);
  }
});

test("internal-link sealing remains incomplete without a persisted rich-text block id", () => {
  const site = { pages: [{ slug: "home", title: "Accueil", blocks: [{ blockType: "rich_text", content: { html: "<p>Accueil</p>" } }] }, { slug: "avis", title: "Avis clients", blocks: [] }] };
  const operation = sealInternalLinkOperation({ type: "add-internal-link", suggestedSourceSlugs: ["home"] }, { pageSlug: "avis", diagnostics: { internalLink: { path: "/agence/gien/avis" } } }, site, site.pages[1]);
