...
async function createCampaign(prisma, plan, name = null) {
  await prisma.$executeRaw`
    INSERT INTO "PresenceCampaign" ("campaignId", "name", "status", "policy", "baseline", "plan")
    VALUES (${plan.campaignId}, ${name}, 'draft', CAST(${json(plan.policy)} AS JSONB), CAST(${json(plan.baseline)} AS JSONB), CAST(${json(plan)} AS JSONB))
  ...
  if (campaign) {
    await prisma.$executeRaw`
      ...
      SELECT ${plan.campaignId}, NULL, 'draft', 'campaign_created', CAST(${json({ selectedCount: plan.selectedCount })} AS JSONB)
