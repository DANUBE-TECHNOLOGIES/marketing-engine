...
function siteHref(site,slug){const root=String(site?.basePath||`/agence/${encodeURIComponent(site?.slug||"")}`).replace(/\/$/,"");const normalized=String(slug||"").trim().replace(/^\/+|\/+$/g,"");return normalized?`${root}/${normalized}`:root;}
...
function localContactIntro(site){const agency=site?.agency||{};const city=String(agency.city||site?.city||"").trim();const nearby=resolvedTargetCities(site,{limit:3});if(!city)return"Retrouvez les coordonnées publiques de votre agence pour échanger avec un conseiller et préparer votre prochain voyage.";const area=nearby.length?` Ce mini-site présente également l’agence pour les secteurs de ${joinCities(nearby)}.`:"";return`Retrouvez les coordonnées publiques de votre agence de voyages à ${city} pour échanger avec un conseiller.${area}`;}

function ReviewExperienceCard({agency,city}){
 const reviewUrl=String(agency?.googleReviewUrl||"").trim();
 const content=<><span className="public-site-agency-icon">★</span><div><small>Votre expérience</small><strong>Vous avez voyagé avec nous ?</strong><p>{city?`Vous pouvez publier un avis Google sur l’agence de ${city}.`:"Vous pouvez publier un avis Google sur notre agence."}</p>{reviewUrl?<span>{city?`Déposer un avis Google sur l’agence de ${city} →`:"Déposer un avis Google →"}</span>:null}</div></>;
 if(!reviewUrl)return <article className="public-site-agency-card">{content}</article>;
 return <a className="public-site-agency-card public-site-agency-card-link" href={reviewUrl} target="_blank" rel="noopener noreferrer" aria-label={city?`Déposer un avis Google sur l’agence de ${city}`:"Déposer un avis Google"}>{content}</a>;
}
...
export default function ContactRenderer({section,site}){
 ...
 return <section id="contact" className="public-site-section public-site-contact"><div className="public-site-container">
  ...
  <div className="public-site-related-links" aria-label={city?`Continuer à préparer votre voyage avec l’agence de ${city}`:"Continuer à préparer votre voyage"}><Link href={siteHref(site,"services")}>{city?`Services publiés par notre agence à ${city}`:"Découvrir nos services voyage"}</Link><Link href={siteHref(site,"destinations")}>{city?`Destinations publiées par notre agence à ${city}`:"Explorer nos destinations"}</Link><Link href={siteHref(site,"inspiration")}>{city?`Conseils et inspirations voyage publiés depuis ${city}`:"Lire nos inspirations voyage"}</Link></div>
 </div></section>;
}
export{joinCities,localContactIntro,phoneHref,siteHref,ReviewExperienceCard};
