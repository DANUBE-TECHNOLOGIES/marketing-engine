  ...
  return (
    <main style={S.main}><div style={S.shell}><Brand /><section style={{ ...S.quizCard, ...(current?.[3]?.length <= 4 && !contact ? S.quizCardCompact : {}) }}>
      ...
      {!contact ? <div style={S.questionWrap}>
        ...
        <div style={S.options}>{current[3].map(([value, label, sub]) => <button type="button" key={value} style={{ ...S.option, ...(answers[current[0]] === value ? S.selected : {}) }} onMouseEnter={(e) => { e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = "0 12px 28px rgba(23,59,52,.09)"; e.currentTarget.style.borderColor = "#c9d7d1"; }} onMouseLeave={(e) => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "0 5px 18px rgba(23,59,52,.035)"; e.currentTarget.style.borderColor = answers[current[0]] === value ? "#a5793d" : "#dce5e1"; }} onClick={() => setAnswer(current[0], value)}><span style={S.optionTitle}>{label}</span><span style={S.optionSub}>{sub}</span><span style={S.optionArrow}>→</span></button>)}</div>
        ...
        {state === "error" && <p style={S.error}>Impossible d'enregistrer votre demande : {result?.error}</p>}<button style={S.cta} disabled={state === "loading" || Boolean(form.phone && !form.phoneProjectContact)}>{state === "loading" ? "Préparation de votre profil…" : "Recevoir mes premières recommandations →"}</button><button type="button" style={S.back} onClick={() => setStep(steps.length - 1)}>← Modifier mes réponses</button>
      </form>}
