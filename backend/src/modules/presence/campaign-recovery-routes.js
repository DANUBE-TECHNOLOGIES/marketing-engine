...
const { getRecoveryStabilization, recordRecoveryManualResolution } = require("./campaign-recovery-stabilization");
...
async function buildRecoveryContext(prisma, campaignId) {
  ...
  const [executions, latestPreflight, deploymentReadiness, state, qualifications, stabilization] = await Promise.all([
  ...
  const operatorState = !qualificationState.complete ? "qualification_required" : stabilization.ready ? "safe_recovery" : "safe_recovery_with_manual_followup";
  return { sourceCampaign, executions, latestPreflight, deploymentReadiness, eligibility, qualificationState, stabilization, activationGate, plan, qualifications, readiness: { ready, decision: ready ? "go" : "no_go", operatorState, blockers } };
}
...
function campaignRecoveryRoutes({ prisma }) {
  ...
  router.get("/api/presence/campaigns/:campaignId/recovery-preview", async (req, res) => {
    try {
      ...
      return res.status(context.readiness.ready ? 200 : 409).json({ ok: context.readiness.ready, externalWrite: false, persisted: false, recoveryOfCampaignId: context.sourceCampaign.campaignId, preflightId: context.latestPreflight?.preflightId || null, readiness: context.readiness, eligibility: context.eligibility, qualificationState: context.qualificationState, stabilization: context.stabilization, activationGate: context.activationGate, qualifications: context.qualifications, plan: context.plan });
  ...
  router.post("/api/presence/campaigns/:campaignId/recovery/resolve", async (req, res) => {
    try {
      if (req.body?.confirm !== true) return res.status(409).json({ ok: false, externalWrite: false, error: "confirm=true requis pour enregistrer la résolution manuelle" });
      const sourceCampaign = await getCampaign(prisma, req.params.campaignId);
      if (!sourceCampaign) return res.status(404).json({ ok: false, externalWrite: false, error: "Campagne Presence introuvable" });
      if (sourceCampaign.status !== "failed") return res.status(409).json({ ok: false, externalWrite: false, error: "La résolution recovery exige une campagne failed" });
      const executions = await listCampaignExecutions(prisma, req.params.campaignId);
      const campaignIndex = Number(req.body?.campaignIndex);
      const execution = executions.find((row) => Number(row.campaignIndex) === campaignIndex);
      if (!execution) return res.status(404).json({ ok: false, externalWrite: false, error: "Item recovery introuvable" });
      const stabilization = await recordRecoveryManualResolution(prisma, { sourceCampaignId: sourceCampaign.campaignId, campaignIndex, agencyId: execution.agencyId || null, operationId: execution.operationId || null, resolution: req.body?.resolution, note: req.body?.note || null });
      return res.json({ ok: true, externalWrite: false, persisted: true, stabilization });
    } catch (error) { return res.status(error.status || 500).json({ ok: false, externalWrite: false, error: error.message }); }
  });
  ...
  router.post("/api/presence/campaigns/:campaignId/recovery", async (req, res) => {
    try {
      ...
      const context = await buildRecoveryContext(prisma, req.params.campaignId);
      if (!context.readiness.ready) return res.status(409).json({ ok: false, externalWrite: false, persisted: false, error: "Recovery gate NO-GO", ...context.readiness, eligibility: context.eligibility, qualificationState: context.qualificationState, stabilization: context.stabilization, activationGate: context.activationGate });
      const source = context.sourceCampaign.approvedScope || {};
      const approvedScope = { agencyIds: context.plan.policy.agencyIds, providerKeys: context.plan.policy.providerKeys, maxItems: context.plan.policy.maxItems, allowSensitive: false, rolloutStage: source.rolloutStage || null, sourceEvidenceCampaignId: source.sourceEvidenceCampaignId || null, sourceEvidenceReportId: source.sourceEvidenceReportId || null, sourceEvidenceReportCreatedAt: source.sourceEvidenceReportCreatedAt || null, recoveryOfCampaignId: context.sourceCampaign.campaignId };
      const approvedPlanFingerprint = stableId({ approvedScope, selected: context.plan.selected || [] });
      const campaign = await createCampaign(prisma, context.plan, req.body?.name || `Reprise ${context.sourceCampaign.name || context.sourceCampaign.campaignId}`, { pilot: true, preflightId: context.latestPreflight.preflightId, approvedScope, approvedPlanFingerprint });
