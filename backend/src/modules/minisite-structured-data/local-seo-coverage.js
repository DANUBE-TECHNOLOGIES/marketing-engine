...
function structuredDataAudit(site, publicOrigin) {
  ...
  return {
    hasTravelAgency: types.includes("TravelAgency"),
    hasLocalBusiness: types.includes("LocalBusiness"),
    hasTelephone: Boolean(node?.telephone),
    hasAddress: Boolean(node?.address?.streetAddress && node?.address?.postalCode && node?.address?.addressLocality),
    hasAreaServed: Boolean(node?.areaServed?.name),
    locality: node?.address?.addressLocality || null,
    areaServed: node?.areaServed?.name || null,
  };
}
...
function localSeoCoverageForSite(site, { targetCities = [], publicOrigin = "https://agences.mondescale.com" } = {}) {
  ...
  return { siteSlug: site?.slug || null, agencyName: agency.name || null, primaryCity: primaryCity || null, targetCities: cities, coveredCities, missingCities, publishedPageCount: publishedPages.length, nap, structuredData, score, status: score >= 85 ? "strong" : score >= 65 ? "improvable" : "weak", gaps, pages: pageAudits };
}
...
function auditLocalSeoCoverage(sites, contexts = {}, options = {}) {
  ...
  return { version: "mse-25.24", summary: { siteCount: items.length, strong: items.filter((item) => item.status === "strong").length, improvable: items.filter((item) => item.status === "improvable").length, weak: items.filter((item) => item.status === "weak").length, averageScore: items.length ? Math.round(items.reduce((sum, item) => sum + item.score, 0) / items.length) : 0, napComplete: items.filter((item) => item.nap.complete).length, structuredDataComplete: items.filter((item) => item.structuredData.hasTravelAgency && item.structuredData.hasLocalBusiness && item.structuredData.hasAddress && item.structuredData.hasTelephone).length }, sites: items };
}

module.exports = { normalizeText, contentText, firstHeading, napAudit, structuredDataAudit, localSeoCoverageForSite, auditLocalSeoCoverage };
