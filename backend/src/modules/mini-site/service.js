...
class MiniSiteService {
  constructor(prisma) { this.prisma = prisma; this.repo = new MiniSiteRepository(prisma); this.generator = new MiniSiteGenerator(); this.destinationGenerator = new DestinationGenerator(); }
  ...
  async createDestinationCluster(miniSiteId, data) {
    ...
    const agency = Number.isInteger(agencyId) ? await this.prisma.agency.findUnique({ where: { id: agencyId } }) : null;
