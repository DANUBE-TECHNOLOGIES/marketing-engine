...
export default async function ConversionIntentPage() {
  ...
  return <main className="min-h-screen bg-gray-100 p-8 text-gray-900"><div className="max-w-7xl mx-auto">
    <PageHeader title="Conversion & Intent — Journey Intelligence" subtitle="MSE-25.47 — performance, parcours commerciaux, transitions, temporalité et points de friction · analyse strictement read-only" />
    ...
    <section className="bg-white rounded-xl shadow p-5 mb-8"><h2 className="font-bold text-lg mb-4">Priorités d’optimisation</h2><div className="space-y-3">{opportunities.slice(0,20).map((item,index) => <article key={`${item.siteSlug}:${item.pageSlug}:${index}`} className="border rounded-xl p-4"><div className="font-semibold">{item.siteSlug} · {item.pageSlug}</div><p className="text-sm text-gray-600 mt-1">{item.recommendation}</p></article>)}{!opportunities.length && <p className="text-sm text-gray-500">Aucune recommandation page-level suffisamment documentée.</p>}</div></section>

    <section className="bg-white rounded-xl shadow p-5 mb-8"><h2 className="font-bold text-lg mb-4">Évolution période sur période</h2><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="text-left border-b"><th className="py-2 pr-3">Agence</th><th className="py-2 pr-3">Page</th><th className="py-2 pr-3 text-right">Avant</th><th className="py-2 pr-3 text-right">Maintenant</th><th className="py-2 text-right">Écart</th></tr></thead><tbody>{comparisons.slice(0,40).map(item => <tr key={`${item.siteSlug}:${item.pageSlug}`} className="border-b last:border-0"><td className="py-2 pr-3">{item.siteSlug}</td><td className="py-2 pr-3">{item.pageSlug}</td><td className="py-2 pr-3 text-right">{rate(item.previousConversionRate)}</td><td className="py-2 pr-3 text-right">{rate(item.currentConversionRate)}</td><td className="py-2 text-right font-semibold">{signedPoints(item.rateDeltaPoints)}</td></tr>)}</tbody></table>{!comparisons.length && <p className="text-sm text-gray-500 py-4">Les tendances apparaîtront lorsque deux périodes seront comparables.</p>}</div></section>

    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-8">
      <section className="bg-white rounded-xl shadow p-5"><h2 className="font-bold text-lg mb-4">Dégradations à investiguer</h2><div className="space-y-2">{degrading.slice(0,15).map(item => <div key={`${item.siteSlug}:${item.pageSlug}`} className="border rounded-lg p-3"><div className="font-semibold">{item.siteSlug} · {item.pageSlug}</div><div className="text-sm text-gray-500">{rate(item.previousConversionRate)} → {rate(item.currentConversionRate)} · {signedPoints(item.rateDeltaPoints)}</div></div>)}{!degrading.length && <p className="text-sm text-gray-500">Aucune dégradation statistiquement exploitable.</p>}</div></section>
      <section className="bg-white rounded-xl shadow p-5"><h2 className="font-bold text-lg mb-4">Améliorations confirmées</h2><div className="space-y-2">{improving.slice(0,15).map(item => <div key={`${item.siteSlug}:${item.pageSlug}`} className="border rounded-lg p-3"><div className="font-semibold">{item.siteSlug} · {item.pageSlug}</div><div className="text-sm text-gray-500">{rate(item.previousConversionRate)} → {rate(item.currentConversionRate)} · {signedPoints(item.rateDeltaPoints)}</div></div>)}{!improving.length && <p className="text-sm text-gray-500">Aucune amélioration suffisamment documentée.</p>}</div></section>
    </div>

    <section className="bg-white rounded-xl shadow p-5 mb-8"><h2 className="font-bold text-lg mb-4">Classement réseau par page comparable</h2><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="text-left border-b"><th className="py-2 pr-3">Page</th><th className="py-2 pr-3">Agence</th><th className="py-2 pr-3 text-right">Rang</th><th className="py-2 text-right">Taux</th></tr></thead><tbody>{rankings.slice(0,60).map(item => <tr key={`${item.siteSlug}:${item.pageSlug}`} className="border-b last:border-0"><td className="py-2 pr-3">{item.pageSlug}</td><td className="py-2 pr-3">{item.siteSlug}</td><td className="py-2 pr-3 text-right">{item.rank}/{item.peerCount}</td><td className="py-2 text-right">{rate(item.conversionRate)}</td></tr>)}</tbody></table>{!rankings.length && <p className="text-sm text-gray-500 py-4">Le classement réseau nécessite un volume comparable suffisant.</p>}</div></section>

    <div className="text-xs text-gray-500">Données first-party uniquement · zéro PII · session anonyme · recommandations déterministes et strictement read-only : aucune page, aucun CTA et aucun contenu public n’est modifié automatiquement. MSE-25.47 observe et priorise ; il n’écrit ni contenu, ni CTA, ni configuration publique.</div>
  </div></main>;
}
