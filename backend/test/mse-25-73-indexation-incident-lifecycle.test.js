...
test("MSE-25.73 marks unseen incidents as NEW", () => {
  const result = evaluateIncidentLifecycle({
    incidents: [{ id: "HTTP_ERROR:https://agences.mondescale.com/agence/dax", code: "HTTP_ERROR", severity: "P0" }],
    previousRecords: [],
...
test("MSE-25.73 marks repeated active incidents as PERSISTING", () => {
  const result = evaluateIncidentLifecycle({
    incidents: [{ id: "A", code: "HTTP_ERROR" }],
    previousRecords: [{ id: "A", active: true, state: "NEW", firstSeenAt: "2026-08-26T20:00:00.000Z", lastSeenAt: "2026-08-26T21:00:00.000Z", occurrenceCount: 1, consecutiveObservationCount: 1 }],
    observedAt: "2026-08-27T00:00:00.000Z",
  });
...
test("MSE-25.73 resolves incidents missing from the new observation", () => {
  const result = evaluateIncidentLifecycle({
    incidents: [],
    previousRecords: [{ id: "A", active: true, state: "PERSISTING", firstSeenAt: "2026-08-26T20:00:00.000Z", lastSeenAt: "2026-08-26T23:00:00.000Z", occurrenceCount: 3, consecutiveObservationCount: 3 }],
    observedAt: "2026-08-27T00:00:00.000Z",
  });
...
test("MSE-25.73 marks a previously resolved incident as REOPENED", () => {
  const result = evaluateIncidentLifecycle({
    incidents: [{ id: "A", code: "CANONICAL_RUNTIME_MISMATCH" }],
    previousRecords: [{ id: "A", active: false, state: "RESOLVED", firstSeenAt: "2026-08-20T00:00:00.000Z", lastSeenAt: "2026-08-21T00:00:00.000Z", resolvedAt: "2026-08-22T00:00:00.000Z", occurrenceCount: 2, reopenCount: 0 }],
    observedAt: "2026-08-27T00:00:00.000Z",
  });
