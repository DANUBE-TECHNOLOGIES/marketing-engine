...
function Cards({ items = [] }) {
  return <div className="as-grid">{items.map((item, i) => {
    const value = typeof item === "string" ? { title: item } : item;
    return <article className="as-card" key={`${value.title}-${i}`}><h3>{value.title}</h3>{value.text && <p>{value.text}</p>}</article>;
  })}</div>;
}

export default function SectionRenderer({ section }) {
  const c = section?.jsonContent || {};
  const type = section?.sectionType;
  if (type === "hero") return <section className="as-hero"><div className="as-shell"><p className="as-eyebrow">{c.eyebrow}</p><h1>{c.title}</h1><p className="as-lead">{c.text}</p><div className="as-actions"><Action action={c.primaryCta}/><Action action={c.secondaryCta} secondary/></div></div></section>;
