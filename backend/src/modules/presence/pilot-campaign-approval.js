...
async function assertPilotCampaignTransition(prisma,campaign,toStatus){
  ...
  const [latestPreflight,currentReadiness]=await Promise.all([getLatestDeploymentPreflight(prisma),buildDeploymentReadiness(prisma)]);if(!latestPreflight||latestPreflight.preflightId!==campaign.preflightId)blockers.push("campaign_preflight_not_latest");const activationGate=evaluatePilotActivationGate({preflight:latestPreflight,currentReadiness});blockers.push(...activationGate.blockers);const sourceGate=await evaluateSourceEvidenceBinding(prisma,campaign,currentReadiness?.network?.agencyCount||0);blockers.push(...(sourceGate.blockers||[]));
  const recoverySource=campaign?.approvedScope?.recoveryOfCampaignId||null;let recoverySnapshot=null;if(recoverySource){recoverySnapshot=await evaluateRecoveryStabilizationSnapshotBinding(prisma,recoverySource,{snapshotId:campaign.approvedScope.recoveryStabilizationSnapshotId||null,evidenceSignature:campaign.approvedScope.recoveryStabilizationEvidenceSignature||null});blockers.push(...(recoverySnapshot.blockers||[]));}
  if(blockers.length){const error=new Error(`Pilot campaign transition blocked: ${[...new Set(blockers)].join(",")}`);error.status=409;error.code="PILOT_CAMPAIGN_GATE_NO_GO";error.blockers=[...new Set(blockers)];error.sourceEvidence=sourceGate;error.recoverySnapshot=recoverySnapshot;throw error;}
}
