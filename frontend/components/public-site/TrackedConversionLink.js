...
export function buildConversionPayload({ conversionType, siteId, siteSlug, paymentVariant, paymentProducts, paymentInstallments, paymentFeeMode, ctaLabel } = {}) {
  return {
...
export function trackConversion(payload) {
  ...
  try {
    window.dispatchEvent(new CustomEvent("mondescale:conversion", { detail: payload }));
  } catch {}
}

export default function TrackedConversionLink({ href, className, children, tracking, ...props }) {
  const payload = buildConversionPayload(tracking);
  return <a {...props} className={className} href={href} onClick={() => trackConversion(payload)} data-conversion-type={payload.conversion_type || undefined}>{children}</a>;
}
