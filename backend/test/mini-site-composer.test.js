...
const { buildRecommendations, composeDestinationPage, composeDestinationSections, buildPageCreateData } = require("../src/lib/miniSiteComposer");
...
test("prépare une création Prisma reliée au mini-site", () => {
  const composed = composeDestinationPage({ destination, site, agency: site.agency });
  const data = buildPageCreateData(composed, site.id);
  assert.deepEqual(data.site, { connect: { id: "site-1" } });
  assert.equal(Array.isArray(data.sections.create), true);
  assert.equal(data.sections.create.length, composed.sections.length);
  assert.equal(Object.prototype.hasOwnProperty.call(data, "siteId"), false);
});

test("refuse une persistance sans identifiant de mini-site", () => {
  const composed = composeDestinationPage({ destination, site, agency: site.agency });
  assert.throws(() => buildPageCreateData(composed, ""), /site id is required/i);
});
